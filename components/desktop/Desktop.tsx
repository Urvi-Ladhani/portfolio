...
export default function Desktop() {
  ...
  const handleOpenDesktopItem = (item: DesktopItem) => {
    if (item.target === 'about') {
      ...
      openWindow('project-resume', 'Resume.pdf', {
        title: 'Resume.pdf',
        description: 'Curriculum Vitae / Resume — Full-Stack Developer & Software Engineer',
  ...
  return (
    <div 
      className="w-screen h-screen p-3 md:p-6 flex items-center justify-center overflow-hidden select-none"
      style={{ backgroundColor: 'var(--color-bg-page, #C9C5F5)' }}
      onClick={() => setSelectedItemId(null)}
    >
      {/* Desktop Surface Frame */}
      <div 
        className="w-full h-full max-w-[1680px] max-h-[1050px] relative flex flex-col rounded-[24px] overflow-hidden shadow-2xl transition-all"
        style={{
          ...
          boxShadow: '0 25px 50px -12px rgba(45, 35, 95, 0.25), 0 0 0 1px rgba(0, 0, 0, 0.05)',
        }}
      >
        {/* Menu Bar */}
        <MenuBar />
